...
from .store.repo_scoped_keys import (
    _assert_repo_scope,
    _backfill_control_plane_local_keys,
    _backfill_control_plane_repo_ids,
    _local_id_after_first_dash,
    _next_repo_sequence,
    _repo_name_for_repo_id,
    _repo_scope_predicate,
    _repo_scoped_sequence_ref,
    _sequence_after_first_dash,
    _sequence_after_last_dash,
)
...
def _refresh_change_state(ctx: ServerContext, conn, change_id: str) -> str:
    ...
    existing_state = str(change["status"])
    if existing_state in {"landed", "archived"}:
...
def get_change_for_repo(ctx: ServerContext, repo_name: str, change_ref: str) -> dict:
    ...
    with connect(ctx) as conn:
        ...
        change_id = str(row["change_id"])
        if _refresh_change_state(ctx, conn, change_id) != str(row["status"]):
            conn.commit()
            row = conn.execute("select * from changes where change_id = ?", (change_id,)).fetchone()
            assert row is not None
