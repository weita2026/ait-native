...
node -e 'const fs=require("node:fs");const marker=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));if(marker.name!=="ait-core"||marker.snapshot!=="SNP-D911094E962F"){throw new Error("ait-core external marker identity drift")}' "$external_core/.ait-external-marker.json"
