...
node -e 'const fs=require("node:fs");const marker=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));if(marker.name!=="ait-core"||marker.snapshot!=="SNP-54923CEF58C8"){throw new Error("ait-core external marker identity drift")}' "$external_core/.ait-external-marker.json"
...
npm test
npm run check
node release/release-adapter.mjs build portable 1.1.0
node release/release-adapter.mjs smoke portable 1.1.0
