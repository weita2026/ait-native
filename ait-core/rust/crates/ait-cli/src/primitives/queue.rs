...
use ait_core::task_workflow_http_adapter::{
    TaskWorkflowQueueSummaryBundleReader, TaskWorkflowReviewerInboxReader,
        ...
        + ?Sized,
{
    let bundle_result = {
        ...
        queue_remote_summary_bundle_with_task_remote(task_remote, repo_name)
    };
    match bundle_result {
        Ok(bundle) => {
        ...
        Err(err) if queue_summary_bundle_missing(&err.to_string()) => {
            match queue_remote_task_queue_with_task_remote(task_remote, repo_name) {
...
{
    task_remote
        .read_queue_summary_bundle(repo_name, Some("active"))
...
{
    task_remote
        .read_task_queue(repo_name, Some("active"))
        .map_err(|err| err.to_string())
}
...
fn queue_remote_section(
    ...
    queue_remote_reads_with_task_remote(&mut task_remote, remote_section, &repo_name)
}

pub fn queue_summary(repo: &RepoRuntime, remote_name: Option<&str>) -> Result<JsonValue, String> {
    ...
    let remote_section = {
        ...
        queue_remote_section(repo, remote_name)?
    };
