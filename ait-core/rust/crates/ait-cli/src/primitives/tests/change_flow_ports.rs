...
#[test]
fn task_publish_remote_create_accepts_task_record_remote_trait() {
    let local_task = json!({
        "task_id": "LCT-PUBLISH",
        "repo_name": "fixture-ait",
        "title": "Publish task",
        "intent": "Exercise task publish helper"
    });
    let mut remote = FakeTaskRecordRemote::default();
    let remote_port: &mut dyn TaskWorkflowTaskRecordRemote = &mut remote;

    let created = super::change_flow::task_publish_remote_create_with_task_remote(
        remote_port,
        "fixture-ait",
        &local_task,
        "LCT-PUBLISH",
    ...
    .expect("publish remote task");
    assert_eq!(created["task_id"], json!("LCT-PUBLISH"));
    assert_eq!(created["plan_id"], json!("PL-1"));

    let mut mismatch = FakeTaskRecordRemote {
        created_task_id_override: Some("RCT-DIFFERENT".to_string()),
        ..Default::default()
    };
    let mismatch_port: &mut dyn TaskWorkflowTaskRecordRemote = &mut mismatch;
    let err = super::change_flow::task_publish_remote_create_with_task_remote(
        mismatch_port,
        "fixture-ait",
        &local_task,
        "LCT-PUBLISH",
    ...
    .expect_err("remote id mismatch should fail");
    assert!(err.contains("must preserve the requested canonical id"));
}

#[test]
fn task_publish_flow_accepts_local_stores_and_task_record_remote_traits() {
    let task_store = FakeTaskStore::default();
    let change_store = FakeChangeStore::default();
    task_local_create_with_task_store(
        &task_store,
        "fixture-ait",
        "Completed local publish task",
        "Exercise task publish flow helper",
        Some("LCT"),
        Some("PL-PUB"),
        Some("PR-PUB"),
        Some("item-pub"),
    )
    .expect("create publish task");
    task_local_close_with_task_store(&task_store, "LCT-1", "completed")
        .expect("complete local publish task");
    change_local_create_with_change_store(
        &change_store,
        "fixture-ait",
        "LCT-1",
        "Landed local publish change",
        "main",
        Some("LCC"),
        Some("SNP-BASE"),
    )
    .expect("create publish change");
    ...
    .expect("land publish change");
    let local_task =
        task_local_read_with_task_store(&task_store, "LCT-1").expect("read publish task");

    let mut rejected_remote = FakeTaskRecordRemote::default();
    let rejected_remote_port: &mut dyn TaskWorkflowTaskRecordRemote = &mut rejected_remote;
    let err = task_publish_with_local_stores_and_task_remote(
        &task_store,
        &change_store,
        rejected_remote_port,
        &local_task,
        "LCT-1",
        "fixture-ait",
        "origin",
    ...
    .expect_err("completed local task with landed change should need promotion");
    assert!(err.contains("already has landed local change"));

    let mut remote = FakeTaskRecordRemote::default();
    let remote_port: &mut dyn TaskWorkflowTaskRecordRemote = &mut remote;
    let published = task_publish_with_local_stores_and_task_remote(
        &task_store,
        &change_store,
        remote_port,
        &local_task,
    ...
    .expect("publish completed local task through promotion helper");
    assert_eq!(published["publication_state"], json!("published"));
    assert_eq!(published["published_remote_name"], json!("origin"));
    assert_eq!(published["published_task_id"], json!("LCT-1"));

    let remote_task = remote.tasks.get("LCT-1").expect("remote task created");
    assert_eq!(remote_task["plan_id"], json!("RPL-PUB"));
    assert_eq!(remote_task["origin_plan_revision_id"], json!("RPR-PUB"));
}
