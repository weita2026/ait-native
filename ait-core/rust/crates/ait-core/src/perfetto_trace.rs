...
mod tests {
    ...
    fn nested_ranges_write_perfetto_complete_events() {
        ...
        let path = temp.path().join("nested-ranges.json");
        let outer = PerfettoRange::with_path("ait.cli.run", Some(path.clone()));
        {
            let _inner = PerfettoRange::with_path("ait.core.read", Some(path.clone()));
        ...
        assert!(events.iter().any(|event| {
            event["name"] == JsonValue::String("ait.cli.run".to_string())
                && event["ph"] == JsonValue::String("X".to_string())
        }));
        assert!(events.iter().any(|event| {
            event["name"] == JsonValue::String("ait.core.read".to_string())
