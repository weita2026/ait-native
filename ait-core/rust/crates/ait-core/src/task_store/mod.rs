...
pub fn list_tasks_with_task_store<S>(store: &S) -> PlanStoreResult<Vec<Value>>
where
    S: TaskStore + ?Sized,
{
    store.list_tasks()
...
pub fn get_task_with_task_store<S>(store: &S, task_id: &str) -> PlanStoreResult<Value>
where
    S: TaskStore + ?Sized,
{
...
pub fn allocate_task_identity_with_task_store<S>(
    store: &S,
    repo_name: &str,
    namespace_prefix: Option<&str>,
) -> PlanStoreResult<Value>
where
    S: TaskStore + ?Sized,
{
    store.allocate_task_identity(repo_name, namespace_prefix)
}

pub fn task_sequence_floor_with_task_store<S>(
    store: &S,
    repo_name: &str,
    family: &str,
) -> PlanStoreResult<i64>
where
    S: TaskStore + ?Sized,
{
    store.sequence_floor(repo_name, family)
}

#[allow(clippy::too_many_arguments)]
pub fn create_task_with_task_store<S>(
    store: &S,
    repo_name: &str,
    title: &str,
    intent: &str,
...
#[allow(clippy::too_many_arguments)]
pub fn create_task_explicit_with_task_store<S>(
    store: &S,
    task_id: &str,
    repo_name: &str,
    title: &str,
...
pub fn close_task_with_task_store<S>(
    store: &S,
    task_id: &str,
    status: &str,
) -> PlanStoreResult<Value>
