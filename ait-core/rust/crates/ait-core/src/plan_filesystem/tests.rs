...
struct SubstituteArtifactResolver;

impl ArtifactResolver for SubstituteArtifactResolver {
    type Error = PlanFilesystemError;

    fn normalize_markdown_artifact_path(&self, path_value: &str) -> String {
        format!("normalized:{path_value}")
    }
    ...
    fn list_visible_workspace_paths(
        ...
        Ok(vec![
            format!("workspace:{repo_root}"),
            format!("ignore:{}", ignore_rules_text.unwrap_or("none")),
            format!("runtime:{}", runtime_root.unwrap_or("none")),
        ])
    }
    ...
    fn zip_archive_has_member(
        ...
        Ok(path_value == "archive.zip" && entry_name == "present.md")
    }
    ...
    fn read_zip_archive_member(
        ...
        Ok(format!("{path_value}:{entry_name}").into_bytes())
    }
}

#[test]
fn plan_artifact_resolver_foundation_implements_trait() {
    assert_artifact_resolver::<PlanArtifactResolverFoundation>();
}

#[test]
fn artifact_resolver_bound_helpers_accept_substitute_resolver() {
    let resolver = SubstituteArtifactResolver;

    assert_eq!(
        normalize_markdown_artifact_path_with_artifact_resolver(&resolver, r"docs\plan.md"),
        r"normalized:docs\plan.md"
    );
    assert!(is_markdown_artifact_path_with_artifact_resolver(
        &resolver, "markdown"
    ));
    assert!(!is_markdown_artifact_path_with_artifact_resolver(
        &resolver,
    ...
    assert!(is_lineage_only_markdown_artifact_path_with_artifact_resolver(&resolver, "lineage"));
    assert!(!is_lineage_only_markdown_artifact_path_with_artifact_resolver(&resolver, "other"));

    assert_eq!(
        list_visible_workspace_paths_with_artifact_resolver(
            &resolver,
            "/repo",
            Some("ignored/"),
            Some("/runtime"),
        ...
        vec![
            ...
            "ignore:ignored/".to_string(),
            "runtime:/runtime".to_string(),
        ]
    );
    assert_eq!(
        list_visible_markdown_artifact_paths_with_artifact_resolver(
            &resolver,
            "/repo",
            None,
            None,
        ...
        vec![
            "markdown:/repo".to_string(),
            "ignore:none".to_string(),
            "runtime:none".to_string(),
        ]
    );
    assert_eq!(
        read_utf8_text_file_with_artifact_resolver(&resolver, "README.md").expect("text"),
        "text:README.md"
    );
    assert_eq!(
        read_json_file_with_artifact_resolver(&resolver, "payload.json").expect("json")["path"],
        JsonValue::String("payload.json".to_string())
    );
    assert_eq!(
        read_binary_file_with_artifact_resolver(&resolver, "blob.bin").expect("bytes"),
        b"blob.bin"
    );
    let resolved =
        resolve_repo_artifact_path_with_artifact_resolver(&resolver, "/repo", "docs.md", true)
            .expect("resolved path");
    assert_eq!(
        resolved["repo_root"],
        JsonValue::String("/repo".to_string())
    );
    assert_eq!(resolved["path"], JsonValue::String("docs.md".to_string()));
    assert_eq!(resolved["allow_missing"], JsonValue::Bool(true));
    assert!(
        zip_archive_has_member_with_artifact_resolver(&resolver, "archive.zip", "present.md")
            .expect("has member")
    );
    assert!(
        !zip_archive_has_member_with_artifact_resolver(&resolver, "archive.zip", "missing.md")
            .expect("missing member")
    );
    assert_eq!(
        read_zip_archive_member_with_artifact_resolver(&resolver, "archive.zip", "docs/plan.md")
            .expect("archive member"),
        b"archive.zip:docs/plan.md"
    );
}

#[test]
fn plan_artifact_resolver_foundation_delegates_path_helpers() {
    let foundation = PlanArtifactResolverFoundation;
    assert_eq!(
        foundation.normalize_markdown_artifact_path(r"docs\plan.md"),
        normalize_markdown_artifact_path(r"docs\plan.md")
    );
    assert!(foundation.is_markdown_artifact_path("docs/plan.MD"));
    assert!(foundation.is_lineage_only_markdown_artifact_path("docs/plan.md"));
    assert!(foundation.is_lineage_only_markdown_artifact_path("README.md"));
}
