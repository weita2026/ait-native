...
#[test]
fn policy_json_normalizes_payload_boundaries() {
    let policy_json = PolicyJson::stateless();

    let payload = policy_json
        .normalize_policy_payload_json(r#"{"policy_id":"team"}"#)
        .unwrap();
    assert_eq!(payload["policy_id"], "team");

    let eval = policy_json
        .normalize_policy_eval_payload(&json!({"decision":"pass"}))
        .unwrap();
    assert_eq!(eval["decision"], "pass");

    let waiver = policy_json
        .normalize_waiver_payload_json(r#"{"waived":true}"#)
        .unwrap();
    assert_eq!(waiver["waived"], true);
...
#[test]
fn policy_json_extracts_closeout_blocker_helpers() {
    let policy_json = PolicyJson::stateless();
    let payload = json!({
        "policy_id": " team ",
        "decision": " block ",
        "checks": [
            {"status": "pass", "label": "Passing check"},
            {"status": "pending", "label": "Tests"},
            {"status": "hard_fail", "name": "Security"},
            {"status": "waived", "label": "License"},
            {"status": "ignored", "label": "Ignored"}
        ]
    });
    let policy = payload.as_object();

    assert_eq!(
        policy_json.optional_policy_id(&payload).as_deref(),
        Some("team")
    );
    assert_eq!(
        policy_json.optional_decision(&payload).as_deref(),
        Some("block")
    );
    assert!(policy_json.policy_has_checks(policy));
    assert_eq!(
        policy_json.blocking_policy_check_labels(policy),
        vec!["Tests", "Security", "License"]
    );
    assert_eq!(
        policy_json.policy_check_statuses(policy),
        vec!["pass", "pending", "hard_fail", "waived", "ignored"]
    );
    assert_eq!(
        policy_json.workflow_land_policy_blocker_detail(policy, None, Some("pending")),
        "Land preflight is blocked by policy requirements: Tests, Security, License, ...."
    );
    assert_eq!(
        policy_json.workflow_land_policy_blocker_detail(None, Some("LAND-1"), Some("block")),
        "Remote land submission `LAND-1` is blocked because policy is currently `block`."
    );
}
