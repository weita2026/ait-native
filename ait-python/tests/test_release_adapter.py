...
def test_release_adapter_maps_all_six_targets_to_exact_abi3_wheels() -> None:
    ...
    assert manifest["package"]["version"] == "1.0.0rc12"
    ...
    assert {
        ...
        target: f"dist/wheels/{expected_wheel_name(target, '1.0.0rc12')}"
...
def write_fixture_wheel(
    ...
    *,
    ...
    dist_info = "ait_python-1.0.0rc12.dist-info"
    local_component = (
...
def test_wheel_normalization_converges_temporary_roots_and_timestamps(
    ...
    normalize_wheel(wheel_a, "1.0.0rc12", timestamp)
    normalize_wheel(wheel_b, "1.0.0rc12", timestamp)
...
def test_wheel_verification_requires_exact_repository_legal_material(
    ...
    target = "aarch64-apple-darwin"
    wheel_name = expected_wheel_name(target, "1.0.0rc12")
    _, timestamp = parse_source_date_epoch("1785761092")
    ...
    normalize_wheel(valid_wheel, "1.0.0rc12", timestamp)
    verify_wheel(valid_wheel, target, "1.0.0rc12")
    ...
    normalize_wheel(altered_wheel, "1.0.0rc12", timestamp)
    with pytest.raises(error_type, match="NOTICE differs"):
        verify_wheel(altered_wheel, target, "1.0.0rc12")
...
def test_release_epoch_and_wheel_controls_fail_closed(
    ...
    with pytest.raises(error_type, match="RECORD or CycloneDX SBOM"):
        normalize_wheel(missing_sbom, "1.0.0rc12", timestamp)
